	...
	public boolean canExtractItem(int slot, ItemStack stack, EnumFacing face)
	{
		...
		else if (slot == 4 && stack != null)
		{
			...
			else
			{
				...
				//check 1.7.10 FluidContainerRegistry TODO no 1.7.10 any longer
	...
	public void buildComplete()
	{
		...
		if (this.buildType == ID.Build.EQUIP_LOOP || this.buildType == ID.Build.SHIP_LOOP)
		{
			for (int i = 0; i < 4; i++)
			{
				...
				//檢查材料是否足夠
				if (item == null || item.getCount() < getBuildRecord(i))
				{
					return;
				}
				//吃掉材料
				...
				{
					item.grow(-getBuildRecord(i));
					//材料用光, 清空該slot
					if (item.getCount() <= 0) itemHandler.setStackInSlot(i, null);
				}
			}
